...
int	is_closed(t_cub3d *data)
{
	...
	while (++y < data->map.row_count)
	{
		...
		{
			if (data->map.grid[y][x] == '0' || data->map.grid[y][x] == 'N'
				|| data->map.grid[y][x] == 'S' || data->map.grid[y][x] == 'E'
				|| data->map.grid[y][x] == 'W' || data->map.grid[y][x] == 'F')
			{
				if (y == 0 || y == data->map.row_count - 1
					|| x == 0 || x == data->map.col_count - 1)
					return (ft_putendl_fd(NO_CLOSED, 2),
						exit_error(data, EXAMPLE), 0);
...
void	indetifier_checker(t_cub3d *data)
{
	...
	if (!data->textures.no || data->textures.no[0] == '1' || !data->textures.so
		|| data->textures.so[0] == '1' || !data->textures.we
		|| data->textures.we[0] == '1' || !data->textures.ea
		|| data->textures.ea[0] == '1' || data->textures.ceiling == -1
